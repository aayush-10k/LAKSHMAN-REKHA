...
function deadmanLapsed(mandate: MandateState, nowS: number): boolean {
  if (mandate.deadmanSeconds === 0) return false; // disabled
  return nowS > mandate.lastHeartbeat + mandate.deadmanSeconds;
}

function effectiveWindowSpent(mandate: MandateState, nowS: number): number {
...
function baseTrace(factSheet: FactSheet, mandate: MandateState, nowMs: number): DecisionTrace {
 ...
 * needs values the injection-safe FactSheet does not carry. It is a pure FNV-1a
 * hash of the request-identifying fields, used only to key/trace decisions.
 */
function decisionId(factSheet: FactSheet, mandate: MandateState): string {
  const canonical = [
