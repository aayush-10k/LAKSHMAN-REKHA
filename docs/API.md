...
| 429 | `RATE_LIMITED` | ~~Lease-renewal griefing defence~~ — **NOT IMPLEMENTED.** No rate limiting exists anywhere in the core, so nothing can currently return this. Attack class 12 is stopped by the fail-closed property only (no leases → spending stops), which is a different and weaker claim |
